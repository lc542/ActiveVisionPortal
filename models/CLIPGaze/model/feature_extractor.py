...
def rescaled_pos_emb(model, new_size):
    pos_emb = model.positional_embedding  # [num_tokens, dim]
    cls_token = pos_emb[:1]
    spatial_tokens = pos_emb[1:]

    num_patches, dim = spatial_tokens.shape
    side = int(math.sqrt(num_patches))
    assert side * side == num_patches, f"Cannot reshape {num_patches} tokens into square"

    token_shape = (side, side)
    a = spatial_tokens.T.contiguous().view(1, dim, *token_shape)
    b = F.interpolate(a, size=new_size, mode='bicubic', align_corners=False)
    b = b.squeeze(0).view(dim, new_size[0] * new_size[1]).T
    return torch.cat([cls_token, b], dim=0)
...
def visual_forward(model, x_inp, extract_layers=(), skip=False, mask=None,token_shape=(14,14)):
    with torch.no_grad():
        ...
        if x.shape[1] != standard_n_tokens:
            ...
            x = x + rescaled_pos_emb(model, new_shape).to(x.dtype)[None, :, :]
...
if __name__=="__main__":
    import clip
    dataset_path='datasets/COCO-Search18/'
    save_path='datasets/COCO-Search18/vit-L-14/'
    ...
    get_image_embedding(dataset_path, save_path, model, device='cuda:0', overwrite=False,
