...
def evaluate(args, model, device = 'cuda:0', im_h=20, im_w=32, project_num=16):
    ...
    if args.condition == 'absent':
        fixation_path = "datasets/COCO-Search18/coco_search18_fixations_TA_test.json"
    ...
    fix_clusters = np.load(join('models/CLIPGaze/data', 'clusters_test.npy'), allow_pickle=True).item()
    ...
    SemSS = get_semantic_seq_score(predictions, fixations_dict, max_len, 'models/Gazeformer/data/SemSS/stuffthing_maps')
    SemFED = get_semantic_ed(predictions, fixations_dict, max_len, 'models/Gazeformer/data/SemSS/stuffthing_maps')
