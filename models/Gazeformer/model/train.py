...
def train(epoch, args, model, SlowOpt, MidOpt, FastOpt, loss_fn_token, loss_fn_y, loss_fn_x, loss_fn_t,
          train_dataloader, model_dir, model_name, device='cuda:0', im_h=20, im_w=32, patch_size=16):
    model.train()
    token_losses = 0
    reg_losses = 0
    t_losses = 0
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        ...
        for batch_imgs, batch_tgt, batch_tgt_padding_mask, batch_tasks, batch_firstfix in tepoch:
            ...
            out_y, out_x = torch.clamp(out_y, min=0, max=im_h * patch_size - 2), torch.clamp(out_x, min=0,
                                                                                             max=im_w * patch_size - 2)
            ...
            reg_loss = (loss_fn_y(out_y.float(), tgt_out[:, :, 0] * fixation_mask).sum(-1) / fixation_mask.sum(
                -1) + loss_fn_x(out_x.float(), tgt_out[:, :, 1] * fixation_mask).sum(-1) / fixation_mask.sum(-1)).mean()
...
def evaluate(model, loss_fn_token, loss_fn_y, loss_fn_x, loss_fn_t, valid_dataloader, device='cuda:0', im_h=20, im_w=32,
             patch_size=16):
    model.eval()
    token_losses = 0
    reg_losses = 0
...
def main(args):
    ...
    model_name = 'gazeformer_' + str(args.num_encoder) + 'E_' + str(args.num_decoder) + 'D_' + str(
        args.batch_size) + '_' + str(args.hidden_dim) + 'd'
    ...
    train_dataset = fixation_dataset(seq_train, img_ftrs_dir=join(gazeformer_data_dir, 'image_features'))
    valid_dataset = fixation_dataset(seq_valid, img_ftrs_dir=join(gazeformer_data_dir, 'image_features'))
    ...
    valid_dataloader = DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=False, num_workers=6,
                                  collate_fn=collate_fn)
    ...
    model = gazeformer(transformer, spatial_dim=(args.im_h, args.im_w), dropout=args.cls_dropout, max_len=args.max_len,
    ...
    tail_params = list(model.transformer.decoder.parameters()) + list(model.generator_y_mu.parameters()) + list(
        model.generator_x_mu.parameters()) + list(model.generator_y_logvar.parameters()) + list(
        model.generator_x_logvar.parameters()) + list(model.querypos_embed.parameters()) + list(
        model.firstfix_linear.parameters())
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        start_time = timer()
        train_token_loss, train_reg_loss, train_t_loss = train(epoch=epoch, args=args, model=model, SlowOpt=SlowOpt,
                                                               FastOpt=FastOpt, MidOpt=MidOpt,
                                                               loss_fn_token=loss_fn_token, loss_fn_y=loss_fn_y,
                                                               loss_fn_x=loss_fn_x, loss_fn_t=loss_fn_t,
                                                               train_dataloader=train_dataloader, model_dir=model_dir,
                                                               model_name=model_name, device=device)
        end_time = timer()

        valid_token_loss, valid_reg_loss, valid_t_loss = evaluate(model=model, loss_fn_token=loss_fn_token,
                                                                  loss_fn_y=loss_fn_y, loss_fn_x=loss_fn_x,
                                                                  loss_fn_t=loss_fn_t,
