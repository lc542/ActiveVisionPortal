...
def get_args_parser_train():
    ...
    parser.add_argument('--transfer-learn', choices=['none', 'search2freeview', 'freeview2search', 'finetune'],
                        default='none', help='setting for transfer learning')
    parser.add_argument('--split', type=int, default=1, help='dataset split for MIT1003/CAT2000 only (default=1)')
    parser.add_argument('--eval-mode', choices=['greedy', 'sample'], type=str, default='greedy',
                        help='whether to sample scanapth or greedily predict scanpath during evaluation (default=greedy)')
...
def train(unknown_args, dataset=None):
    # parser.add_argument('--dataset-root', type=str, default=os.path.join(PROJECT_ROOT, 'datasets/COCO-Search18'), help='Root path to dataset')
    print("\n=== HAT Train ===")
