...
def build(hparams, dataset_root, device, is_pretraining, is_eval=False, split=1):
    ...
    if is_pretraining:
        ...
        if not hparams.Data.saliency_pred:
            with open(join(dataset_root, 'coco_search18_fixations_512x320_on_target_allvalid.json')) as json_file:
        ...
        if dataset_name == 'OSIE':
            ...
            with open(
                    join(dataset_root,
                         'coco_search18_fixations_512x320_on_target_allvalid.json'), 'r'
            ) as json_file:
                human_scanpaths = json.load(json_file)
    ...
    #         new_state_dict = {}
    #         for k, v in ckp['model'].items():
    #             new_k = k.replace('stages.', '') if 'stages.' in k else k
    ...
    if dataset_name == 'COCO-Search18' or dataset_name == 'COCO-Freeview':
        sss_strings = np.load(join(hparams.Data.sem_seq_dir,
                                   'test.pkl'),
                              allow_pickle=True)
    else:
        sss_strings = None
