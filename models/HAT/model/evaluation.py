...
def evaluate(model,
    ...
    if TAP == 'TP':
        ...
        print(f"[Info] Search CDF: {mean_cdf}")
        print(f"[Info] Prob mismatch: {np.sum(np.abs(human_cdf[:len(mean_cdf)] - mean_cdf))}")

    # sequence score
    print("[Info] Computing sequence scores...")
    ...
    print(
        f"[SeqScore] {cut1} steps: {ss_2steps:.4f}, {cut2} steps: {ss_4steps:.4f}, {cut3} steps: {ss_6steps:.4f}, max: {ss:.4f}")
    ...
    if semSS_strings is not None and TAP != 'FV':
        print("[Info] Computing semantic sequence scores...")
        sss_2steps = metrics.get_semantic_seq_score(
            nonstop_scanpaths, semSS_strings, cut1, 
            f'{pa.sem_seq_dir}/segmentation_maps', True)
        ...
        sss = metrics.get_semantic_seq_score(
            ...
            f'{pa.sem_seq_dir}/segmentation_maps', False)
        print(
            f"[SemanticSS] {cut1} steps: {sss_2steps:.4f}, {cut2} steps: {sss_4steps:.4f}, {cut3} steps: {sss_6steps:.4f}, max: {sss:.4f}")
        metrics_dict.update({
    ...
    print(f"[SP Length Error] Mean: {sp_len_diff.mean():.4f}, Std: {sp_len_diff.std():.4f}")
