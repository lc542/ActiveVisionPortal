...
def parse_args():
    ...
    parser.add_argument('--hparams',
                        type=str,
                        default='./configs/coco_search18_dense_SSL_TP.json',
...
def compute_loss(model, batch, losses, loss_funcs, pa, use_focal_loss, args):
...
def train_iter(model, optimizer, batch, losses, loss_weights, loss_funcs, pa, hptrain, args):
    assert len(losses) > 0, "no loss func assigned!"
    model.train()
    optimizer.zero_grad()

    loss_dict = compute_loss(model, batch, losses, loss_funcs, pa, hptrain.use_focal_loss, args)
    loss = 0
...
def get_eval_loss(model, eval_dataloader, losses, loss_funcs, pa, hptrain, args):
    with torch.no_grad():
        model.eval()
        num_batches = 0
...
def main(args=None):
    ...
    global args_global, hparams, hparams_tp, hparams_ta, hparams_fv
    global dataset_root, output_saliency_metrics, device, sample_action
    global model, optimizer, train_gaze_loader, val_gaze_loader, train_img_loader
    global valid_img_loader_tp, valid_img_loader_ta, valid_img_loader_fv
    global global_step, bbox_annos, human_cdf, fix_clusters
    global prior_maps_tp, prior_maps_ta, prior_maps_fv, sss_strings
    global valid_gaze_loader_tp, valid_gaze_loader_ta, valid_gaze_loader_fv
    global sps_test_tp, sps_test_ta, sps_test_fv, term_pos_weight, log_dir
        ...
        for i_epoch in range(s_epoch, int(1e5)):
            ...
            for i_batch, batch in enumerate(train_gaze_loader):
                loss_dict = train_iter(model, optimizer, batch, losses,
                                       loss_weights, loss_funcs, hparams.Data, hparams.Train, args_global)
                for k in loss_dict:
                ...
                if global_step % eval_every == eval_every - 1:
                    if args.pretrain:
                        loss_dict_avg = get_eval_loss(model, val_gaze_loader,
                                                      losses, loss_funcs,
                                                      hparams_tp.Data, hparams_tp.Train, args_global)
                        log_dict(writer, loss_dict_avg, global_step, 'eval')
                    else:
